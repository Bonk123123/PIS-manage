...
const ClientPage = () => {
  ...
  React.useEffect(() => {
    ...
    console.log(users);
    if (search !== "") {
      let user = users.filter((item) => item.id === parseInt(search));
      setNewUsers(user);
    }
  }, [search]);
  ...
  return (
    <div className="flex w-full h-full">
      ...
      <span
        className=" text-3xl top-3 right-3 cursor-pointer"
      ...
      <div className="flex-col">
        <div className="flex-col">
          <Link className="w-full border flex" to={"/orders"}>
            все заказы
          </Link>
          <Link className="w-full border flex" to={"/onlyData"}>
            данные
          </Link>
          <Link className="w-full border flex" to={"/products"}>
            продукты
          </Link>
        </div>
        <input
          className="border w-full h-10 flex"
          value={search}
